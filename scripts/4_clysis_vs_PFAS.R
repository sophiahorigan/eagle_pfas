...
for(pfas in pfas_cols){
  ...
  df_model <- df %>% filter(!is.na(.data[[pfas]]) & !is.na(log_clysis)) %>% mutate(PFAS_scaled = scale(.data[[pfas]]))
 
  formula_text <- paste0("log_clysis ~ PFAS_scaled + ", paste(covariates, collapse=" + "), " + (1|", nest_var,")")
  ...
  tidy_res <- broom.mixed::tidy(model) %>% filter(term=="PFAS_scaled") %>%
    mutate(pfas_var = pfas, immune_var = "Clysis", n_obs = nrow(df_model))
