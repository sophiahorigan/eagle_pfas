##########################################
# Figure: Key take-homes from Script 2 (clysis)
...
shape_file <- "./output/2_clysis_pfas_relationship/clysis_best_shape_summary.csv"
out_dir <- "./output/2_clysis_pfas_relationship/figures"
...
forced_predictors <- c("TOTAL_PFAS", "func_carboxyl", "func_amine", "age_next_gen")
...
data_clean <- data %>% filter(!is.na(clysis), clysis > 0)

# ---------- Build predictors exactly like Script 2 ----------
...
ggsave(
  filename = file.path(out_dir, "Fig_2_key_takehomes_logClysis_panel.png"),
  plot = p, width = 10, height = 10, dpi = 600
)

ggsave(
  filename = file.path(out_dir, "Fig_2_key_takehomes_logClysis_panel.tiff"),
  plot = p, width = 10, height = 10, dpi = 600, compression = "lzw"
)

write.csv(
  selected,
  file.path(out_dir, "Fig_2_key_takehomes_selected_predictors.csv"),
  row.names = FALSE
)
...
if (nrow(supp_selected) > 0) {
  ...
  if (nrow(supp_plot_dat) > 0 && nrow(supp_curve_dat) > 0) {
    ...
    ggsave(
      filename = file.path(out_dir, "Fig_S2_supp_all_significant_logClysis_panel.png"),
      plot = p_supp, width = 12, height = supp_height, dpi = 600
    )

    ggsave(
      filename = file.path(out_dir, "Fig_S2_supp_all_significant_logClysis_panel.tiff"),
      plot = p_supp, width = 12, height = supp_height, dpi = 600, compression = "lzw"
    )

    write.csv(
      supp_selected,
      file.path(out_dir, "Fig_S2_supp_all_significant_selected_predictors.csv"),
      row.names = FALSE
    )
  }
}

cat("Done: key take-home panel figure for Script 2\n")
