...
# ========= SETTINGS =========
out_dir <- "./output/14_weight_resid_foot_pfas_relationship"
...
# ========= RESIDUALIZE WEIGHT BY FOOT SIZE =========
fit_idx <- complete.cases(data$weight_kg, data$foot_mm)
if (sum(fit_idx) < 8) stop("Not enough observations for weight_kg ~ foot_mm residualization.")

size_model <- lm(weight_kg ~ foot_mm, data = data[fit_idx, ])
data$weight_resid_foot <- NA_real_
data$weight_resid_foot[fit_idx] <- residuals(size_model)

data_clean <- data %>% filter(!is.na(weight_resid_foot))

# ========= IDENTIFY PFAS =========
pfas_present <- intersect(key$pfas_analyte, colnames(data_clean))
if (length(pfas_present) == 0) stop("No PFAS analyte columns found in data.")
...
run_shape_models <- function(response_var, response_label) {
  ...
  for (pfas_var in pfas_vars) {
    ...
    df <- data_model %>%
      ...
      filter(!is.na(.data[[pfas_var]]), !is.na(.data[[response_var]]))
    ...
    y_min_pred <- predict(best_model, newdata = data.frame(
      x = min(df$x),
      log_x = log(min(df$x) + 1e-6),
      x2 = min(df$x)^2,
      x_adj = min(df$x) + 1e-6
    ))
    y_max_pred <- predict(best_model, newdata = data.frame(
      x = max(df$x),
    ...
    results_list[[row_key]] <- data.frame(
      PFAS = pfas_var,
    ...
    plot_list[[row_key]] <- list(
      ...
      file_path = file.path(raw_plot_dir, paste0(pfas_var, "_", response_label, "_shape.jpeg"))
    )
  }
...
# ========= RUN =========
results <- run_shape_models("weight_resid_foot", "weight_resid_foot")
all_results <- do.call(rbind, results$results)

# ========= MULTIPLE TEST CORRECTION =========
all_results$FDR_P <- p.adjust(all_results$P_value, method = "BH")

# ========= PLOT =========
for (row_key in names(results$plots)) {
  plot_info <- results$plots[[row_key]]
...
cat("Done: weight residual (weight_kg ~ foot_mm) vs PFAS shape modeling\n")
