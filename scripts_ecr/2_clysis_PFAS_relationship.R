...
sig_results <- all_results %>%
  filter((P_value < 0.05 | FDR_P < 0.05), Response == "log_clysis")
...
if (nrow(sig_results) > 0) {
  sig_plot_data <- lapply(seq_len(nrow(sig_results)), function(i) {
    ...
    y_vals <- data_model$clysis
    if (resp_label == "log_clysis") {
  ...
  if (nrow(sig_plot_data) > 0) {
    ...
    p_sig_sex <- ggplot(sig_plot_data, aes(x = x, y = y)) +
      ...
      labs(
        title = "Significant Complement Lysis-PFAS Relationships by Sex",
        x = "PFAS concentration / category sum",
        y = "Complement lysis (raw or log-transformed)"
      ) +
