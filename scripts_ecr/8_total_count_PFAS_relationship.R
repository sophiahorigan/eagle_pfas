...
# ========= SETTINGS =========
out_dir <- "./output/8_total_count_pfas_relationship"
...
data_clean <- data %>% filter(!is.na(Total_count))
...
run_shape_models <- function(response_var, response_label) {
  ...
  for (pfas_var in pfas_vars) {
    ...
    if (response_label == "log_Total_count") {
      df <- df %>% filter(y > 0)
      df$y <- log(df$y)
    }
...
# ========= RUN =========
results_raw <- run_shape_models("Total_count", "Total_count_raw")
results_log <- run_shape_models("Total_count", "log_Total_count")
...
print("Total_count modeling complete.")
