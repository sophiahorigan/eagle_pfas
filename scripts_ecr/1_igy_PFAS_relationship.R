...
# ========= SETTINGS =========
out_dir <- "./output/1_igy_pfas_relationship"
...
# ========= LOAD DATA =========
data <- read.csv("./input/2023_2024_2025_AllData_clean.csv", stringsAsFactors = FALSE)
...
# ========= ADDITIONAL FIGURE: SIGNIFICANT MODELS FACETTED BY SEX (M/F) =========
sig_results <- all_results %>%
  filter((P_value < 0.05 | FDR_P < 0.05), Response == "log_IgY")

if (nrow(sig_results) > 0) {
  sig_plot_data <- lapply(seq_len(nrow(sig_results)), function(i) {
    pfas_var <- sig_results$PFAS[i]
    resp_label <- sig_results$Response[i]
    if (!pfas_var %in% colnames(data_model)) return(NULL)
    
    y_vals <- data_model$igy
    if (resp_label == "log_IgY") {
      y_vals <- ifelse(y_vals > 0, log(y_vals), NA_real_)
    }
    
    data.frame(
      PFAS = pfas_var,
      Response = resp_label,
      Best_Model = sig_results$Best_Model[i],
      Sex = toupper(trimws(data_model$field_sex)),
      x = data_model[[pfas_var]],
      y = y_vals
    )
  })
  
  sig_plot_data <- bind_rows(sig_plot_data) %>%
    filter(Sex %in% c("M", "F"), !is.na(x), !is.na(y)) %>%
    mutate(
      Sex = factor(Sex, levels = c("F", "M")),
      Facet = paste0(PFAS, " | ", Response)
    )
  
  if (nrow(sig_plot_data) > 0) {
    n_facets <- length(unique(sig_plot_data$Facet))
    ncol_facets <- 5
    nrow_facets <- ceiling(n_facets / ncol_facets)
    
    # Fit sex-specific lines using the model form selected in the primary best-shape workflow.
    line_df_list <- lapply(
      split(sig_plot_data, list(sig_plot_data$Facet, sig_plot_data$Sex), drop = TRUE),
      function(d) {
        if (nrow(d) < 6 || dplyr::n_distinct(d$x) < 4) return(NULL)

        model_name <- as.character(d$Best_Model[1])
        d <- d %>%
          mutate(
            log_x = log(x + 1e-6),
            x2 = x^2,
        ...
        fit <- switch(
          model_name,
          linear = try(lm(y ~ x, data = d), silent = TRUE),
          log = try(lm(y ~ log_x, data = d), silent = TRUE),
          quadratic = try(lm(y ~ x + x2, data = d), silent = TRUE),
          exp = try(nls(y ~ a * exp(b * x),
                        start = list(a = mean(d$y), b = -0.01),
                        data = d), silent = TRUE),
          power = try(nls(y ~ a * x_adj^b,
                          start = list(a = mean(d$y), b = -0.5),
                          data = d), silent = TRUE),
          try(lm(y ~ x, data = d), silent = TRUE)
        )
        if (inherits(fit, "try-error")) return(NULL)

        new_x <- seq(min(d$x), max(d$x), length.out = 100)
        pred_df <- data.frame(
          x = new_x,
          log_x = log(new_x + 1e-6),
          x2 = new_x^2,
        ...
        data.frame(
          ...
          Sex = d$Sex[1],
          x = new_x,
          y = as.numeric(predict(fit, newdata = pred_df))
        )
      }
    )
    line_df <- bind_rows(line_df_list)
    
    p_sig_sex <- ggplot(sig_plot_data, aes(x = x, y = y)) +
      geom_point(alpha = 0.55, size = 1.2, color = "gray35") +
      geom_line(
        data = line_df,
        aes(x = x, y = y, color = Sex, group = interaction(Facet, Sex)),
        linewidth = 1
      ) +
      scale_color_manual(
        name = "Sex",
        values = c("F" = "red3", "M" = "dodgerblue3"),
        labels = c("Female", "Male"),
        breaks = c("F", "M")
      ) +
      facet_wrap(~ Facet, scales = "free", ncol = ncol_facets) +
      labs(
        title = "Significant IgY-PFAS Relationships by Sex",
        x = "PFAS concentration / category sum",
        y = "IgY (raw or log-transformed)"
      ) +
    ...
    ggsave(
      filename = file.path(out_dir, "significant_models_faceted_by_sex.jpeg"),
      plot = p_sig_sex,
      width = 18, height = max(10, min(24, nrow_facets * 2.8)), dpi = 300
    )
  }
}
...
write.csv(all_results,
          file.path(out_dir, "best_shape_summary.csv"),
          row.names = FALSE)
