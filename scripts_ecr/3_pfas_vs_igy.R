...
best_shape_file <- "./output/1_igy_pfas_relationship/best_shape_summary.csv"
...
# Load best-fit shapes from Script 1 (log_IgY models)
...
for(pfas in pfas_cols){
  ...
  df_model <- df %>%
    filter(!is.na(.data[[pfas]]) & !is.na(log_igy)) %>%
    mutate(
      PFAS_raw = .data[[pfas]],
